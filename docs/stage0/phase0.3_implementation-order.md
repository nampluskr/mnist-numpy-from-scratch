...
| Stage 2 | MNIST 데이터 로더 | `data/mnist.py`, `data/dataloader.py` |
| Stage 3 | NumPy MLP | `models/mlp.py`, `layers.py`, `activations.py`, `losses.py` |
| Stage 4 | 실행 객체 | `core/` 7개 파일 |
...
| `src/data/dataloader.py` | `DataLoader` | dataset, batch_size, shuffle | iterable |
| `src/models/mlp.py` | `MLP` | config 또는 차원 인자 | model instance |
| `src/models/activations.py` | `sigmoid`, `softmax`, `identity`, `relu` | `ndarray` | `ndarray` |
| `src/models/losses.py` | `cross_entropy`, `binary_cross_entropy`, `mse` | `preds, targets: ndarray` | scalar |
| `src/core/optimizers.py` | `SGD`, `Adam` | model instance, lr | optimizer instance |
...
- `SGD.step()`, `Adam.step()` : model.params 를 in-place 업데이트, 반환값 없음

## 3. TDD 원칙

Stage 1부터 구현보다 테스트를 먼저 작성한다. 실패 테스트 작성 원칙은 다음과 같다.

- public interface 기준으로 테스트를 먼저 작성하고, 내부 helper 는 필요한 경우에만 간접 검증한다.
- 파일 단위로 진행하며, 해당 파일의 대응 테스트가 먼저 실패하는지 확인한 뒤 구현한다.
- unit test를 먼저 작성하고, 조합이 필요한 경우에만 통합 테스트를 추가한다.
- fixture 는 재사용 가치가 있을 때만 `tests/conftest.py` 로 올리고, 파일 전용 데이터는 각 테스트 파일에 둔다.
