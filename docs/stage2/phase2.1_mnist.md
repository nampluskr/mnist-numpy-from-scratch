...
| 클래스 | 테스트 | 검증 내용 |
|---|---|---|
| `TestLoadMnist` | `test_train_images_shape` | `"train"` split image shape `(N, 28, 28)` |
| `TestLoadMnist` | `test_train_labels_shape` | `"train"` split labels shape `(N,)` |
| `TestLoadMnist` | `test_images_dtype_uint8` | image dtype `uint8` |
| `TestLoadMnist` | `test_labels_dtype_uint8` | label dtype `uint8` |
| `TestLoadMnist` | `test_test_split_shape` | `"test"` split shape |
| `TestLoadMnist` | `test_image_pixel_range` | 픽셀값 범위 0–255 |
| `TestLoadMnist` | `test_label_values` | 레이블값 0–9 |
| `TestLoadMnist` | `test_invalid_split_raises` | 잘못된 split → `ValueError` |
| `TestLoadMnist` | `test_missing_file_raises` | 파일 없음 → `FileNotFoundError` |
| `TestLoadMnistReal` | `test_train_shape` | 실제 데이터 train shape `(60000, 28, 28)` |
| `TestLoadMnistReal` | `test_test_shape` | 실제 데이터 test shape `(10000, 28, 28)` |
| `TestLoadMnistReal` | `test_train_dtypes` | 실제 데이터 dtype |
| `TestLoadMnistReal` | `test_label_range` | 실제 데이터 레이블 범위 0–9 |

실 MNIST 데이터 의존 테스트(`TestLoadMnistReal`)는 `/mnt/d/datasets/mnist` 경로가 없으면 skip 처리된다.
