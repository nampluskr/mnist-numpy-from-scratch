...
`src/core/visualizer.py`는 학습 로그와 예측 결과를 시각화하고 output_dir에 PNG 파일로 저장한다.
`Experiment.run()`이 반환한 로그 목록과 `Predictor.predict()`가 반환한 예측값을 입력으로 받는다.
...
### 2.2. plot_training_log(logs, filename="training_log.png")

`Experiment.run()`이 반환한 per-epoch 로그 목록을 받아 loss/metric 곡선을 그린다.
...
### 2.3. plot_predictions(images, labels, predictions, filename="predictions.png", n=16)
...
# 학습 곡선
logs = exp.run()
viz.plot_training_log(logs, filename="training_log.png")
...
|---|---|---|
| `TestPlotTrainingLog` | 5 | 파일 생성, 파일 비어있지 않음, 반환 경로 str, 커스텀 파일명, 1 epoch 처리 |
...
```bash
conda run -n numpy_env pytest tests/stage4/test_visualizer.py -v
```
...
- `output_dir`은 `Path.mkdir(parents=True, exist_ok=True)`로 생성하므로 중간 경로가 없어도 동작한다.
