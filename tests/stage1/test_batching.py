...
def test_get_batches_iterable():
    batches = list(get_batches(x, batch_size=BATCH_SIZE))
    assert len(batches) > 0


def test_get_batches_single_array_yields_array():
    for batch in get_batches(x, batch_size=BATCH_SIZE):
        assert isinstance(batch, np.ndarray)
        break


def test_get_batches_multiple_arrays_yields_tuple():
...
def test_get_batches_batch_size():
    for batch in get_batches(x, batch_size=BATCH_SIZE):
        assert batch.shape[0] <= BATCH_SIZE
        break


def test_get_batches_last_batch_smaller():
    batches = list(get_batches(x, batch_size=BATCH_SIZE))
    # N=20, batch_size=6 → batches of size 6,6,6,2
    assert batches[-1].shape[0] == N % BATCH_SIZE


def test_get_batches_covers_all_samples():
    total = sum(b.shape[0] for b in get_batches(x, batch_size=BATCH_SIZE))
    assert total == N


def test_get_batches_no_shuffle_preserves_order():
...
def test_get_batches_multiple_arrays_consistent_shuffle():
    np.random.seed(0)
