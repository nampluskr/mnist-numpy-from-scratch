...
def test_len_exact_division(ds):
    loader = DataLoader(ds, batch_size=4, shuffle=False)
    assert len(loader) == 5


def test_len_with_remainder(ds):
    loader = DataLoader(ds, batch_size=6, shuffle=False)
    assert len(loader) == math.ceil(20 / 6)  # 4


def test_len_batch_size_larger_than_dataset():
    ds = ToyDataset(n=5)
...
def test_iter_yields_tuples(ds):
    loader = DataLoader(ds, batch_size=4, shuffle=False)
    batch = next(iter(loader))
    assert isinstance(batch, tuple) and len(batch) == 2


def test_iter_batch_image_shape(ds):
    loader = DataLoader(ds, batch_size=4, shuffle=False)
    images, _ = next(iter(loader))
    assert images.shape == (4, 4)


def test_iter_batch_target_shape(ds):
    loader = DataLoader(ds, batch_size=4, shuffle=False)
    _, targets = next(iter(loader))
    assert targets.shape == (4, 1)


def test_iter_correct_num_batches(ds):
    loader = DataLoader(ds, batch_size=4, shuffle=False)
    batches = list(loader)
    assert len(batches) == 5


def test_iter_last_batch_smaller(ds):
    loader = DataLoader(ds, batch_size=6, shuffle=False)
    batches = list(loader)
...
def test_iter_total_samples(ds):
...
def test_shuffle_changes_order(ds):
...
def test_shuffle_covers_all_samples(ds):
    loader = DataLoader(ds, batch_size=20, shuffle=True)
    images, _ = next(iter(loader))
...
def test_multiple_iter_independent(ds):
    loader = DataLoader(ds, batch_size=20, shuffle=True)
