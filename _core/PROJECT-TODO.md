...
tags: [project, docs]
created: 2026-06-08
updated: 2026-06-20 (task 기술 형식 통일)
---
...
- [x] [[docs/stage0/phase0.0_conda-setup|phase0.0_conda-setup.md]] 문서 작성
...
- [x] `requirements.txt` 구현
- [x] `src/config.py` 구현
- [x] `tests/stage1/test_config.py` 작성
...
- [x] `src/task.py` 구현
- [x] `tests/stage1/test_task.py` 작성
...
## 4. Stage 3 nn 모듈 및 모델 구현

### 4.1. Phase 3.1 공통 모듈 구현

`src/nn/activations.py`, `src/nn/losses.py`, `src/nn/metrics.py` 세 파일에 공통 수학 함수를 구현하고 테스트로 검증한다.
...
### 4.3. Phase 3.3 MLP 구현

`Module` 기반 `Linear`, `Sigmoid`, `ReLU`, `Sequential` 레이어(training/train/eval 포함)와 MLP 모델을 구현하고 테스트로 검증한다.
...
- [x] `tests/stage3/test_mlp.py` 작성

### 4.4. Phase 3.4 MLP 문서

MLP 관련 레이어 모듈과 모델에 대한 참조 문서를 작성한다.

- [x] [[docs/stage3/phase3.4_layers|phase3.4_layers.md]] 문서 작성
- [x] [[docs/stage3/phase3.5_mlp|phase3.5_mlp.md]] 문서 작성

### 4.5. Phase 3.5 CNN 구현

`im2col`/`col2im` 기반 `src/nn/conv.py`와 `src/models/cnn.py`를 구현하고, `src/core/experiment.py`에 `config["model"]` CNN 분기를 추가하여 통합 테스트로 검증한다.

- [x] `src/nn/conv.py` 구현 (`im2col`/`col2im` + `Conv2d`, `MaxPool2d`, `Flatten`, `Dropout`)
- [x] `tests/stage3/test_cnn.py` 작성
- [x] `src/models/cnn.py` 구현
- [x] `src/core/experiment.py` 구현 (`config["model"]` CNN 분기 추가)
- [x] `tests/stage3/test_experiment.py` 작성 (CNN 통합 케이스)

### 4.6. Phase 3.6 CNN 문서

CNN 레이어와 모델에 대한 참조 문서를 작성한다.

- [x] [[docs/stage3/phase3.6_conv|phase3.6_conv.md]] 문서 작성
- [x] [[docs/stage3/phase3.7_cnn|phase3.7_cnn.md]] 문서 작성

### 4.7. Phase 3.7 Stage 3 노트북 작성

nn 모듈과 MLP, CNN을 실습하는 교육용 노트북을 작성한다.
...
- [x] `notebooks/stage3/stage3-5_cnn-architecture.ipynb` 작성
- [x] `notebooks/stage3/stage3-6_cnn-training.ipynb` 작성
...
- [x] `tests/stage4/test_optimizers.py` 작성
...
- [x] `src/core/checkpoints.py` 구현
- [x] `tests/stage4/test_checkpoints.py` 작성
...
- [x] `tests/stage4/test_trainer.py` 작성
...
- [x] `tests/stage4/test_evaluator.py` 작성
...
- [x] `tests/stage4/test_predictor.py` 작성
...
- [x] `src/core/experiment.py` 구현
- [x] `tests/stage4/test_experiment.py` 작성
- [x] [[docs/stage4/phase4.6_experiment|phase4.6_experiment.md]] 문서 작성

### 5.7. Phase 4.7 Visualizer 및 training plot helper 구현
...
- [x] `tests/stage4/test_visualizer.py` 작성
...
- [x] `notebooks/stage4/stage4-1_optimizers.ipynb` 작성
- [x] `notebooks/stage4/stage4-2_trainer-and-evaluator.ipynb` 작성
- [x] `notebooks/stage4/stage4-3_experiment.ipynb` 작성

## 6. Stage 5 클라이언트 코드 구현

### 6.1. Phase 5.1 training CLI 구현

커맨드라인 인수를 파싱하여 `Experiment`를 조립하고 `Trainer.fit`을 호출하는 `scripts/train.py` CLI를 구현한다.

- [x] `scripts/train.py` 구현
...
- [x] `notebooks/stage5/stage5-1_cli-scripts.ipynb` 작성

## 7. Stage 6 documentation 및 verification

### 7.1. Phase 6.1 experiment 실행 및 result 수집
...
- [x] `outputs/multiclass/mlp/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] `outputs/multiclass/cnn/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] `outputs/binary/mlp/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] `outputs/binary/cnn/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] `outputs/regression/mlp/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] `outputs/regression/cnn/` (`training_log.png`, `predictions.png`, `model.npz`) 생성
- [x] [[docs/stage6/phase6.1_results|phase6.1_results.md]] 문서 작성

### 7.2. Phase 6.2 Multiclass tutorial

10클래스 분류 task에서 MLP와 CNN의 학습 절차, CLI 실행 명령, 평가 결과를 각각 tutorial 문서로 작성한다.

- [x] [[docs/stage6/phase6.2_tutorial-mlp|phase6.2_tutorial-mlp.md]] 문서 작성
- [x] [[docs/stage6/phase6.2_tutorial-cnn|phase6.2_tutorial-cnn.md]] 문서 작성

### 7.3. Phase 6.3 Binary tutorial

이진 분류 task에서 MLP와 CNN의 학습 절차, sigmoid threshold 후처리, CLI 실행 명령, 평가 결과를 각각 tutorial 문서로 작성한다.

- [x] [[docs/stage6/phase6.3_tutorial-mlp|phase6.3_tutorial-mlp.md]] 문서 작성
- [x] [[docs/stage6/phase6.3_tutorial-cnn|phase6.3_tutorial-cnn.md]] 문서 작성

### 7.4. Phase 6.4 Regression tutorial

회귀 task에서 MLP와 CNN의 학습 절차, R² 평가 지표, round_clip 후처리, CLI 실행 명령, 평가 결과를 각각 tutorial 문서로 작성한다.

- [x] [[docs/stage6/phase6.4_tutorial-mlp|phase6.4_tutorial-mlp.md]] 문서 작성
- [x] [[docs/stage6/phase6.4_tutorial-cnn|phase6.4_tutorial-cnn.md]] 문서 작성

### 7.5. Phase 6.5 framework 연계 준비

본 프로젝트의 모듈·인터페이스 규약을 검토하고, PyTorch 마이그레이션 시 대응 항목을 checklist 문서로 정리하여 후속 프레임워크 프로젝트 시작 기준을 마련하고 문서로 확정한다.

- [x] [[docs/stage6/phase6.5_framework-checklist|phase6.5_framework-checklist.md]] 문서 작성

### 7.6. Phase 6.6 Stage 6 노트북 작성

3종 태스크 전체 실험(MLP + CNN 비교)을 실습하는 교육용 노트북 3개를 작성한다.

- [x] `notebooks/stage6/stage6-1_multiclass-experiment.ipynb` 작성
- [x] `notebooks/stage6/stage6-2_binary-experiment.ipynb` 작성
- [x] `notebooks/stage6/stage6-3_regression-experiment.ipynb` 작성
