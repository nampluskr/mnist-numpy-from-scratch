...
tags: [project, docs]
created: 2026-06-08
updated: 2026-06-20 (신규 Stage 0~7 구조로 전면 재작성)
---
...
- [ ] `src/data/mnist.py` 구현
- [ ] `tests/stage2/test_mnist.py` 작성
- [ ] [[docs/stage2/phase2.1_mnist|phase2.1_mnist.md]] 문서 작성

### 3.2. Phase 2.2 Dataset 클래스 구현

`MnistDataset` 클래스를 구현하고, task에 따른 target 변환과 `__getitem__` 인터페이스를 갖추어 DataLoader와 연동 가능하게 한다.

- [ ] `src/data/mnist.py` 구현 (`MnistDataset` 추가)
- [ ] `tests/stage2/test_dataset.py` 작성
- [ ] [[docs/stage2/phase2.2_dataset|phase2.2_dataset.md]] 문서 작성

### 3.3. Phase 2.3 DataLoader 구현

배치 생성, shuffle, 반복 순회를 지원하는 `DataLoader`를 구현하고, `MnistDataset`과의 통합 동작을 검증한다.

- [ ] `src/data/dataloader.py` 구현
- [ ] `tests/stage2/test_dataloader.py` 작성
- [ ] [[docs/stage2/phase2.3_dataloader|phase2.3_dataloader.md]] 문서 작성
...
### 4.1. Phase 3.1 activation 함수

`sigmoid`, `softmax`, `identity`, `relu` 활성화 함수를 구현한다.

- [ ] `src/nn/activations.py` 구현
- [ ] `tests/stage3/test_activations.py` 작성
- [ ] [[docs/stage3/phase3.1_activations|phase3.1_activations.md]] 문서 작성

### 4.2. Phase 3.2 loss 함수

`cross_entropy`, `binary_cross_entropy`, `mse` 손실 함수와 gradient 함수를 구현한다.

- [ ] `src/nn/losses.py` 구현
- [ ] `tests/stage3/test_losses.py` 작성
- [ ] [[docs/stage3/phase3.2_losses|phase3.2_losses.md]] 문서 작성

### 4.3. Phase 3.3 metric 함수

`accuracy`, `binary_accuracy`, `r2_score` 평가 지표를 구현한다.

- [ ] `src/nn/metrics.py` 구현
- [ ] `tests/stage3/test_metrics.py` 작성
- [ ] [[docs/stage3/phase3.3_metrics|phase3.3_metrics.md]] 문서 작성

### 4.4. Phase 3.4 MLP 레이어 구현

`Module` 기반 `Linear`, `Sigmoid`, `ReLU`, `Sequential` 레이어(training/train/eval 포함)를 구현한다.

- [ ] `src/nn/layers.py` 구현 (`Module` + `Linear`, `Sigmoid`, `ReLU`, `Sequential`, training/train/eval 포함)
- [ ] `tests/stage3/test_layers.py` 작성
- [ ] [[docs/stage3/phase3.4_layers|phase3.4_layers.md]] 문서 작성

### 4.5. Phase 3.5 CNN 레이어 구현

`im2col`/`col2im` 기반 `Conv2d`, `MaxPool2d`, `Flatten`, `Dropout`을 구현한다.

- [ ] `src/nn/conv.py` 구현 (`im2col`/`col2im` + `Conv2d`, `MaxPool2d`, `Flatten`, `Dropout`)
- [ ] `tests/stage3/test_conv.py` 작성
- [ ] [[docs/stage3/phase3.5_conv|phase3.5_conv.md]] 문서 작성
...
### 5.1. Phase 4.1 MLP 모델

`src/nn/` 모듈을 조립하여 NumPy 기반 MLP를 구현한다.

- [ ] `src/models/mlp.py` 구현
- [ ] `tests/stage4/test_mlp.py` 작성 (이동 예정: `tests/stage3/test_mlp.py`)
- [ ] [[docs/stage4/phase4.1_mlp|phase4.1_mlp.md]] 문서 작성

### 5.2. Phase 4.2 CNN 모델

CuPy 기반 CNN 모델을 구현한다.

- [ ] `src/models/cnn.py` 구현
- [ ] `tests/stage4/test_cnn.py` 작성 (이동 예정: `tests/stage3/test_cnn.py`에서 모델 관련 분리)
- [ ] [[docs/stage4/phase4.2_cnn|phase4.2_cnn.md]] 문서 작성
...
### 6.1. Phase 5.1 optimizer

`SGD`와 `Adam` optimizer를 구현하고, 각 parameter에 대한 update 규칙과 모멘텀/적응 학습률 동작을 테스트로 검증한다.

- [ ] `src/core/optimizers.py` 구현
- [ ] `tests/stage5/test_optimizers.py` 작성 (이동 예정: `tests/stage4/test_optimizers.py`)
- [ ] [[docs/stage5/phase5.1_optimizers|phase5.1_optimizers.md]] 문서 작성
...
- [ ] `src/core/trainer.py` 구현
- [ ] `tests/stage5/test_trainer.py` 작성 (이동 예정: `tests/stage4/test_trainer.py`)
- [ ] `src/core/evaluator.py` 구현
- [ ] `tests/stage5/test_evaluator.py` 작성 (이동 예정: `tests/stage4/test_evaluator.py`)
- [ ] [[docs/stage5/phase5.2_trainer-evaluator|phase5.2_trainer-evaluator.md]] 문서 작성
...
- [ ] `src/core/predictor.py` 구현
- [ ] `tests/stage5/test_predictor.py` 작성 (이동 예정: `tests/stage4/test_predictor.py`)
- [ ] `src/core/visualizer.py` 구현
- [ ] `tests/stage5/test_visualizer.py` 작성 (이동 예정: `tests/stage4/test_visualizer.py`)
- [ ] [[docs/stage5/phase5.3_predictor-visualizer|phase5.3_predictor-visualizer.md]] 문서 작성
...
- [ ] `tests/stage5/test_logger.py` 작성
- [ ] [[docs/stage5/phase5.4_logger|phase5.4_logger.md]] 문서 작성
...
- [ ] `scripts/train.py` 작성
- [ ] `tests/stage6/test_train.py` 작성
- [ ] `scripts/evaluate.py` 작성
- [ ] `tests/stage6/test_evaluate.py` 작성
- [ ] [[docs/stage6/phase6.1_train-evaluate|phase6.1_train-evaluate.md]] 문서 작성
...
- [ ] `tests/stage6/test_predict.py` 작성
- [ ] `scripts/visualize.py` 작성
- [ ] `tests/stage6/test_visualize.py` 작성
- [ ] [[docs/stage6/phase6.2_predict-visualize|phase6.2_predict-visualize.md]] 문서 작성
...
- [ ] `experiments/run_all.py` 작성
- [ ] [[docs/stage6/phase6.3_run-all|phase6.3_run-all.md]] 문서 작성
...
- [ ] [[docs/stage6/phase6.4_experiments-and-results|phase6.4_experiments-and-results.md]] 문서 작성
